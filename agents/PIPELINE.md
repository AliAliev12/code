...
- **Footer / `technical_pages`**: состав юрстраниц в футере и правила ключей (умеренно, без копирования наборов между сайтами; необязательность вставки ключей в **H2**/**H3** — **только** для страниц из **`technical_pages`**) — см. **`agents/prompts/cazilla-site-factory-ru.md`** (раздел **«Футер и технические страницы»**).
