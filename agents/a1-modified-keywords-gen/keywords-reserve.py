...
ENV_PATH = ROOT / ".env"
INPUT_PATH = Path("./keywords-reserve.txt")   # <-- входной файл со списком ключей
...
if site_dir:
    out_path = ROOT / site_dir / "_output" / "keywords-reserve.json"
else:
    out_path = ROOT / "agents/a1-keywords/agents/output/keywords-reserve.json"

out_path.parent.mkdir(parents=True, exist_ok=True)
allow_overwrite = (env.get("ALLOW_KEYWORDS_OVERWRITE", "").strip().lower() in {"1", "true", "yes", "on"})
if out_path.exists() and not allow_overwrite:
    raise SystemExit(
        f"Refusing to overwrite existing keywords file: {out_path}\n"
        "Use ALLOW_KEYWORDS_OVERWRITE=1 only when overwrite is intentional."
    )
